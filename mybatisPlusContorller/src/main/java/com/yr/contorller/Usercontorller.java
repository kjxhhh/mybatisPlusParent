...
public class Usercontorller {
    ...
    public String getUsers(Map<String, Object> map, Integer pageNo, @RequestParam(defaultValue = "-1") Integer pages) { // 查询
        ...
        IPage<User> users = userService.getUsers(pageNo);
        map.put("users", users.getRecords());

        map.put("Total", users.getTotal()); // 总条数
        map.put("Pages", users.getPages()); // 总页数
        map.put("Current", users.getCurrent()); // 当前页数
        map.put("Size", users.getSize()); // 显示条数
        return "list";
    }
    ...
    public String execute(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, int id) {
        ...
        try {
            ...
            // inputStream = new
            // FileInputStream("D:\\springmvc\\Springmvc_spring\\WebContent\\head\\20191111113156418.jpg");
